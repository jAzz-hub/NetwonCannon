...
//para tentar deixar tudo com uma escala real 
// constante gravitacional double constanteGravitacional = 6.674e-11;
// massa da lua double massaLua = 7.35e22;
// massa da terra double massaTerra = 5.972e24;
...
int spritePlaneta, spriteProjetil, spriteCanhao, spriteTorre, spriteCeu, spriteSolo;
int timerGeral;

Vetor rastro[QTD_MAX_RASTRO];
int qtdRastro;

void adicionarProjetil(Vetor posicao, Vetor velocidade, Vetor aceleracao,
                       double massa, double raio) {
  int i = qtdProjeteisAtuais;


// massa da lua para usar como referencia
// double massaLua = 7.35e22;
...
void desenharProjeteis() {
  for (int i = 0; i < qtdProjeteisAtuais; i++) {
    ...
    //                    YVirtualParaReal(projeteis[i].posicao.x,
    //                    projeteis[i].posicao.y),
    ...
    DesenharSprite(
        ...
        tamanhoVirtualParaReal(2.0 * projeteis[i].raio),
        tamanhoVirtualParaReal(2.0 * projeteis[i].raio), 0);
...
void desenharRastro() {
  for (int i = 0; i < qtdRastro - 1; i++) {
    ...
    DesenharLinhaSimples(XVirtualParaReal(xAtual), YVirtualParaReal(yAtual),
                         XVirtualParaReal(xProximo), YVirtualParaReal(yProximo),
                         VERDE);
  }
}
...
void desenhar() {
  IniciarDesenho();
  // void DesenharSprite(int id_sprite, int X, int Y, int Largura, int Altura,
  // float Angulo, int referencialVirtual = 1)

  // DesenharSprite(spriteCeu,0,0,LARG_TELA,ALT_TELA,0,0,BRANCO);
  DesenharSprite(spriteCeu, LARG_TELA/2, ALT_TELA/2, LARG_TELA, ALT_TELA, 0, 0);
  DesenharSprite(spriteSolo,680, -200, LARG_TELA, ALT_TELA, 0, 0);
  ...
  //                    tamanhoVirtualParaReal(2.0*planeta.raio),
  //                    0, 1, BRANCO);
  DesenharSprite(spritePlaneta, XVirtualParaReal(planeta.posicao.x),
                 YVirtualParaReal(planeta.posicao.y),
                 tamanhoVirtualParaReal(2.0 * planeta.raio),
                 tamanhoVirtualParaReal(2.0 * planeta.raio), 0, 1);
  ...
  DesenharSprite(spriteCanhao, XVirtualParaReal(50),
                 YVirtualParaReal(310),
                 tamanhoVirtualParaReal(100), tamanhoVirtualParaReal(100), 0, 1);
  ...
  DesenharSprite(spriteTorre, XVirtualParaReal(70),
                 YVirtualParaReal(230),
                 tamanhoVirtualParaReal(500),
                 tamanhoVirtualParaReal(500), 0, 1);
...
void aplicarColisao(Corpo* p1, Corpo* p2) {
  ...
  p1->velocidade.x = v1x;
  p1->velocidade.y =
      v1y;  // Retornando o resultado final para as variaveis das esfera
...
void atualizarProjeteis() {
  ...
  for (int i = 0; i < qtdProjeteisAtuais; i++) {
    ...
    double distancia =
        DistanciaEntrePontos(xProjetil, yProjetil, xPlaneta, yPlaneta);
    if (distancia < 1) {
      distancia = 1;
    }
    ...
    if (DistanciaEntrePontos(projeteis[i].posicao.x, projeteis[i].posicao.y,
                             xPlaneta,
                             yPlaneta) < raioProjetil + raioPlaneta * 0.982) {
...
int main(int argc, char* args[]) {
  ...
  while (PIG_JogoRodando == 1) {
    if (TempoDecorrido(timerGeral) >= 0.10) {
